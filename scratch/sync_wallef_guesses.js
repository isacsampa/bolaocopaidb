...
const SHEET_GUESSES = [
  ...
  { time_a: "Japão", time_b: "Tunísia", a: 1, b: 1 }, 
  { time_a: "Japão", time_b: "Suécia", a: 2, b: 0 }, 
...
async function run() {
  ...
  console.log("🔄 Iniciando sincronização dos palpites de Wallef...");
  ...
  // 2. Montar lista de upsert
  const upsertData = [];

  SHEET_GUESSES.forEach(sheetBet => {
    ...
    upsertData.push({
      usuario_id: wallefId,
      jogo_id: dbGame.id,
      palpite_gols_a: targetA,
      palpite_gols_b: targetB
    });
  });

  console.log(`💾 Executando upsert de ${upsertData.length} palpites...`);
  const { data, error } = await supabase
    .from("palpites")
    .upsert(upsertData, { onConflict: "usuario_id,jogo_id" })
    .select();

  if (error) {
    console.error("❌ Erro ao sincronizar palpites:", error.message);
  } else {
    console.log(`✅ Sincronização concluída com sucesso! ${data.length} palpites atualizados no banco.`);
  }
}
